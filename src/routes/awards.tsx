...
export const Route = createFileRoute("/awards")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Confirm winning providers and issue work orders after requester selection." },
      { property: "og:title", content: "Award Management — Quotalink" },
      { property: "og:description", content: "Confirm winning providers and issue work orders after requester selection." },
    ],
  }),
  component: Awards,
});
...
const awards = [
  { id: "AW-1042", rfq: "RFQ-24185", requester: "Kingsford Manufacturing", provider: "Bramwell Facilities Ltd", value: "£61,400", stage: "Requester Selected", eta: "Confirm within 24h" },
  { id: "AW-1041", rfq: "RFQ-24183", requester: "Meridian Financial", provider: "Silverline Contracts", value: "£18,200", stage: "Work Order Draft", eta: "Awaiting signature" },
  { id: "AW-1040", rfq: "RFQ-24179", requester: "Ashbury Property Trust", provider: "Clyde & Ross Mechanical", value: "£33,900", stage: "Confirmed", eta: "Provider notified" },
  { id: "AW-1039", rfq: "RFQ-24175", requester: "Whitmore Retail Group", provider: "Highfield Engineering", value: "£52,600", stage: "Confirmed", eta: "Provider notified" },
];

const stageTone = (s: string) => s === "Requester Selected" ? "warning" : s === "Work Order Draft" ? "info" : s === "Confirmed" ? "success" : "muted";

function Awards() {
  const controller = useAwardsController();

  return (
    <AppShell title="Award Management" subtitle="Finalize requester decisions and generate work orders.">
        ...
        {[
          ...
          return (
            <Card key={k.label} className="p-4 flex items-center gap-4">
              <div className="w-11 h-11 rounded-lg bg-primary/10 text-primary grid place-items-center"><Icon className="w-5 h-5" /></div>
              <div>
      ...
      <Card>
        <div className="p-5 pb-3"><SectionHeader title="Award pipeline" hint="Selections returning from requesters" /></div>
              ...
              {awards.map((a) => (
                <tr key={a.id} className="hover:bg-secondary/40">
                  ...
                  <td className="px-3 py-3">
                    <div><Pill tone={stageTone(a.stage) as never}>{a.stage}</Pill></div>
                    <div className="text-[11px] text-muted-foreground mt-1">{a.eta}</div>
                  </td>
                  <td className="px-5 py-3 text-right">
                    <Button size="sm"><Award className="w-3.5 h-3.5" />{a.stage === "Confirmed" ? "View" : "Confirm"}</Button>
                  </td>
                </tr>
              ))}
            </tbody>
